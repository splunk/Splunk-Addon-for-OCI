...
if os.getenv("OCI_PYTHON_SDK_NO_SERVICE_IMPORTS", "").lower() in ["true", "1"]:
    ...
    if sys.version_info >= (3, 7) and os.getenv("OCI_PYTHON_SDK_LAZY_IMPORTS_DISABLED", "False").lower() != "true":
        ...
        from . import adm, analytics, announcements_service, apigateway, apm_config, apm_control_plane, apm_synthetics, apm_traces, application_migration, appmgmt_control, artifacts, audit, autoscaling, certificates, certificates_management, cims, cloud_bridge, cloud_guard, cloud_migrations, dashboard_service, data_catalog, data_connectivity, data_flow,  disaster_recovery, email, events, file_storage, functions, governance_rules_control_plane, healthchecks, integration, key_management, license_manager, limits, load_balancer, lockbox, log_analytics, logging, loggingingestion, loggingsearch, management_agent, management_dashboard, network_firewall, network_load_balancer, nosql, opensearch, operator_access_control, queue, recovery, resource_search, rover, sch, secrets, service_catalog, service_manager_proxy, stack_monitoring, streaming, tenant_manager_control_plane, threat_intelligence, usage, usage_api, vault, vbs_inst, visual_builder, vn_monitoring, work_requests
