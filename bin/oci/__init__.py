...
if os.getenv("OCI_PYTHON_SDK_NO_SERVICE_IMPORTS", "").lower() in ["true", "1"]:
    ...
    if sys.version_info >= (3, 7) and os.getenv("OCI_PYTHON_SDK_LAZY_IMPORTS_DISABLED", "False").lower() != "true":
        ...
        from . import adm, analytics, announcements_service, apigateway, artifacts, audit, certificates, certificates_management, cims, dashboard_service, email, events, file_storage, functions, healthchecks, log_analytics, logging, loggingingestion, loggingsearch, management_agent, management_dashboard, opensearch, operator_access_control, queue, resource_search, rover, sch, secrets, service_catalog, service_manager_proxy, streaming, threat_intelligence, usage, usage_api, vault, work_requests
